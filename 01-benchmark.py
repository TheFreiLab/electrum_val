...
if __name__ == '__main__':
    ...
    for name, fp_func in fingerprints.items():
        ...
        X = np.array(fp_func(df['LigandSmiles'], df['Metal'], radius=2, n_bits=512))
        ...
        std = df_true.std() * 100
        summary[name] = [f'{m:.1f} ± {s:.2f}' for m, s in zip(mean, std)]
        ...
        summary[f'{name}_scrambled'] = [f'{m:.1f} ± {s:.2f}' for m, s in zip(mean_s, std_s)]
