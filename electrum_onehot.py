...
def one_hot_encode_metals(metals: list):
    """
    One-hot encodes a list of metal strings and returns the encoding plus a metal-to-index mapping.

    Parameters:
    ----------
    metals (list of str): List of metal symbols (e.g. ['Fe', 'Cu', 'Zn'])

    Returns:
    ----------
    np.ndarray: One-hot encoded 2D array (shape: [n_samples, n_unique_metals])
    dict: Mapping from metal symbol to column index in one-hot array
    """
    encoder = OneHotEncoder(sparse_output=False, dtype=int)
    one_hot = encoder.fit_transform(np.array(metals).reshape(-1, 1))
    metals_dict = dict(zip(metals, one_hot))
    return metals_dict

metals_dict = one_hot_encode_metals(metals_list)
